...
- The environment variable `GEMINI_API_KEY` must be set (or specified in the skill directory's `.env` file)
...
```bash
uv run --directory . python generate_image.py "prompt"
```
...
```bash
uv run --directory . python generate_image.py "a cat playing piano in watercolor style"
```

High-resolution wide image:
```bash
uv run --directory . python generate_image.py "sunset coastline" --resolution 4K --aspect 16:9
```

Output to specific directory:
```bash
uv run --directory . python generate_image.py "logo design" --output ./assets/images
```
...
```bash
# Change background
uv run --directory . python generate_image.py "change background to sunset" --reference ./original.png

# Use multiple reference images (pose, style, etc.)
uv run --directory . python generate_image.py "draw this person in this pose" --reference ./person.png --reference ./pose.png
```
